# RMRKEquipRenderUtilsUpgradeable

*RMRK team*

> RMRKEquipRenderUtilsUpgradeable

Smart contract of the upgradeable RMRK Equip render utils module.
...
```solidity
function composeEquippables(address target, uint256 tokenId, uint64 assetId) external view returns (string metadataURI, uint64 equippableGroupId, address catalogAddress, struct RMRKEquipRenderUtilsUpgradeable.FixedPart[] fixedParts, struct RMRKEquipRenderUtilsUpgradeable.EquippedSlotPart[] slotParts)
```
...
| fixedParts | RMRKEquipRenderUtilsUpgradeable.FixedPart[] | An array of fixed parts respresented by the `FixedPart` structs present on the asset |
| slotParts | RMRKEquipRenderUtilsUpgradeable.EquippedSlotPart[] | An array of slot parts represented by the `EquippedSlotPart` structs present on the asset |
...
```solidity
function getAllEquippableSlotsFromParent(address targetChild, uint256 childId, bool onlyEquipped) external view returns (uint256 childIndex, struct RMRKEquipRenderUtilsUpgradeable.EquippableData[] equippableData)
```
...
| equippableData | RMRKEquipRenderUtilsUpgradeable.EquippableData[] | An array of `EquippableData` structs containing info about the equippable child assets and their corresponding slot parts |
...
```solidity
function getChildrenWithTopMetadata(address parentAddress, uint256 parentId) external view returns (struct RMRKEquipRenderUtilsUpgradeable.ChildWithTopAssetMetadata[])
```
...
| Name | Type | Description |
|---|---|---|
| _0 | RMRKEquipRenderUtilsUpgradeable.ChildWithTopAssetMetadata[] | An array of `ChildWithTopAssetMetadata` structs representing the children with their top asset metadata |

### getEquippableSlotsFromParent

```solidity
function getEquippableSlotsFromParent(address targetChild, uint256 childId, uint64 parentAssetId) external view returns (uint256 childIndex, struct RMRKEquipRenderUtilsUpgradeable.EquippableData[] equippableData)
```
...
```solidity
function getEquippableSlotsFromParentForPendingChild(address targetChild, uint256 childId, uint64 parentAssetId) external view returns (uint256 childIndex, struct RMRKEquipRenderUtilsUpgradeable.EquippableData[] equippableData)
```
...
```solidity
function getExtendedActiveAssets(address target, uint256 tokenId) external view returns (struct RMRKMultiAssetRenderUtilsUpgradeable.ExtendedActiveAsset[])
```
...
| Name | Type | Description |
|---|---|---|
| _0 | RMRKMultiAssetRenderUtilsUpgradeable.ExtendedActiveAsset[] | An array of ActiveAssets present on the given token |

### getExtendedEquippableActiveAssets

```solidity
function getExtendedEquippableActiveAssets(address target, uint256 tokenId) external view returns (struct RMRKEquipRenderUtilsUpgradeable.ExtendedEquippableActiveAsset[])
```
...
| Name | Type | Description |
|---|---|---|
| _0 | RMRKEquipRenderUtilsUpgradeable.ExtendedEquippableActiveAsset[] | An array of ExtendedEquippableActiveAssets present on the given token |

### getExtendedNft

```solidity
function getExtendedNft(uint256 tokenId, address targetCollection) external view returns (struct RMRKRenderUtilsUpgradeable.ExtendedNft data)
```
...
| Name | Type | Description |
|---|---|---|
| data | RMRKRenderUtilsUpgradeable.ExtendedNft | The `ExtendedNft` struct containing the specified token&#39;s data |

### getExtendedPendingAssets

```solidity
function getExtendedPendingAssets(address target, uint256 tokenId) external view returns (struct RMRKEquipRenderUtilsUpgradeable.ExtendedPendingAsset[])
```
...
| Name | Type | Description |
|---|---|---|
| _0 | RMRKEquipRenderUtilsUpgradeable.ExtendedPendingAsset[] | An array of ExtendedPendingAssets present on the given token |
...
```solidity
function getPendingAssets(address target, uint256 tokenId) external view returns (struct RMRKMultiAssetRenderUtilsUpgradeable.PendingAsset[])
```
...
| Name | Type | Description |
|---|---|---|
| _0 | RMRKMultiAssetRenderUtilsUpgradeable.PendingAsset[] | An array of PendingAssets present on the given token |
...
```solidity
function getTopAssetAndEquippableDataForToken(address target, uint256 tokenId) external view returns (struct RMRKEquipRenderUtilsUpgradeable.ExtendedEquippableActiveAsset topAsset)
```
...
| Name | Type | Description |
|---|---|---|
| topAsset | RMRKEquipRenderUtilsUpgradeable.ExtendedEquippableActiveAsset | `ExtendedEquippableActiveAsset` struct with the equippable data containing the asset with the  highest priority |

# RMRKMultiAssetEmotableMockUpgradeable
...
### __RMRKMultiAssetEmotableMockUpgradeable_init

```solidity
function __RMRKMultiAssetEmotableMockUpgradeable_init(string name, string symbol) external nonpayable
```
...
```solidity
function emote(uint256 tokenId, bytes4 emoji, bool on) external nonpayable
```

...
```solidity
function bulkEmote(address[] collections, uint256[] tokenIds, bytes4[] emojis, bool[] states) external nonpayable
```
...
| emojis | bytes4[] | An array of unicode identifiers of the emojis |
| states | bool[] | An array of boolean values signifying whether to emote (`true`) or undo (`false`) emote |

### bulkEmoteCountOf

```solidity
function bulkEmoteCountOf(address[] collections, uint256[] tokenIds, bytes4[] emojis) external view returns (uint256[])
```
...
```solidity
function bulkPrepareMessagesToPresignEmote(address[] collections, uint256[] tokenIds, bytes4[] emojis, bool[] states, uint256[] deadlines) external view returns (bytes32[])
```
...
| tokenIds | uint256[] | An array of IDs of the tokens being emoted |
| emojis | bytes4[] | An arrau of unicode identifiers of the emojis |
| states | bool[] | An array of boolean values signifying whether to emote (`true`) or undo (`false`) emote |
...
```solidity
function bulkPresignedEmote(address[] emoters, address[] collections, uint256[] tokenIds, bytes4[] emojis, bool[] states, uint256[] deadlines, uint8[] v, bytes32[] r, bytes32[] s) external nonpayable
```
...
```solidity
function haveEmotersUsedEmotes(address[] emoters, address[] collections, uint256[] tokenIds, bytes4[] emojis) external view returns (bool[])
```
...
| emojis | bytes4[] | An array of the ASCII emoji codes being checked for reactions |
...
```solidity
function prepareMessageToPresignEmote(address collection, uint256 tokenId, bytes4 emoji, bool state, uint256 deadline) external view returns (bytes32)
```
...
```solidity
function presignedEmote(address emoter, address collection, uint256 tokenId, bytes4 emoji, bool state, uint256 deadline, uint8 v, bytes32 r, bytes32 s) external nonpayable
```

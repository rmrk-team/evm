  ...
  */
  ...
  describe('Add resource', async function () {
    it('Adds resource', async function () {
      ...
      await rmrkNft.connect(owner).addResourceToToken(1, resourceStorage.address, resArr[0]['id']);
    ...
    it('Add multiple resources and reorder priorities', async function () {
      ...
      await rmrkNft.connect(owner).addResourceToToken(1, resourceStorage.address, resArr[1]['id']);
      ...
      await expect(rmrkNft.connect(addrs[0]).setPriority(1, [resArr[1]['id']])).to.be.revertedWith(
      ...
      await expect(
        rmrkNft
          .connect(addrs[0])
          .setPriority(1, [
            ethers.utils.hexZeroPad('0xaaaaa', 8),
            ethers.utils.hexZeroPad('0xbbbb', 8),
          ]),
      ).to.be.revertedWith('RMRK: Trying to reprioritize a non-existant resource');

      await expect(
        rmrkNft.connect(owner).addResourceToToken(1, resourceStorage.address, resArr[1]['id']),
      ).to.be.revertedWith('RMRKCore: Resource already exists on token');

      await expect(
        rmrkNft
          .connect(owner)
          .addResourceToToken(1, resourceStorage.address, ethers.utils.hexZeroPad('0xa1a2a3', 8)),
    ...
    it('Accept resource', async function () {
      ...
      await expect(rmrkNft.connect(addrs[1]).acceptResource(1, resArr[0]['id'])).to.be.revertedWith(
        'RMRK: Attempting to accept a resource in non-owned NFT',
      );

      await expect(
        rmrkNft.connect(addrs[0]).acceptResource(10, resArr[0]['id']),
      ).to.be.revertedWith('RMRK: resource does not exist');

      await expect(rmrkNft.connect(addrs[0]).acceptResource(1, resArr[1]['id'])).to.be.revertedWith(
        'RMRK: resource does not exist',
      );

      await expect(rmrkNft.connect(addrs[0]).acceptResource(1, resArr[0]['id'])).to.be.revertedWith(

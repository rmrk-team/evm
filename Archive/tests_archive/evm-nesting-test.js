...
describe('init', async function () {
  ...
  describe('Minting', async function () {
    ...
    it('Nest mint non-contract', async function () {
      await expect(
        rmrkNft.connect(owner).doMintNest(owner.address, 1, 0, nestFlag),
      ).to.be.revertedWith('Is not contract');
    });

    it('Nest mint contract, non-existent token', async function () {
      await expect(
        rmrkNft.connect(owner).doMintNest(rmrkNft2.address, 1, 0, nestFlag),
    ...
    it('Nest mint contract, owner and not owner check pending', async function () {
      ...
      expect(pendingChildren).to.eql([
        [ethers.BigNumber.from(1), rmrkNft.address, 0, ethers.utils.hexZeroPad('0x0', 8)],
      ]);
      ...
      children = await rmrkNft2.childrenOf(destId);
      expect(children).to.eql([
        [ethers.BigNumber.from(2), rmrkNft.address, 0, ethers.utils.hexZeroPad('0x0', 8)],
      ]);
      pendingChildren = await rmrkNft2.pendingChildrenOf(destId);
      expect(pendingChildren).to.eql([]);
    });
    ...
    it('Nest mint contract, child management', async function () {
      ...
      pendingChildren = await rmrkNft2.pendingChildrenOf(destId);
      expect(pendingChildren).to.eql([
        [ethers.BigNumber.from(3), rmrkNft.address, 0, ethers.utils.hexZeroPad('0x0', 8)],
      ]);
    ...
    it('Unparent child', async function () {
      ...
      await expect(
        rmrkNft2.connect(addrs[1]).removeChild(destId, rmrkNft.address, 1),
      ).to.be.revertedWith('RMRKCore: bad owner');
    ...
    it('Mint child into child', async function () {
      ...
      expect(children2).to.eql([
        [ethers.BigNumber.from(21), rmrkNft2.address, 0, ethers.utils.hexZeroPad('0x0', 8)],
      ]);
    });

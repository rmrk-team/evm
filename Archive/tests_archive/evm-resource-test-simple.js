  ...
  */
  ...
  describe('Add resource', async function () {
    ...
    it('Accept resource', async function () {
      ...
      expect(await rmrkNft.getRenderableResource(1)).to.eql([
        resourceStorage.address,
        resArr[0]['id'],
      ]);
    });
    ...
    it('Add multiple resources and reorder priorities', async function () {
      ...
      await expect(rmrkNft.connect(addrs[0]).setPriority(1, [resId16_2])).to.be.revertedWith(
    ...
    it('Overwrite Resource', async function () {
      ...
      await rmrkNft
        .connect(owner)
        .addResourceToToken(1, resourceStorage.address, resArr[2]['id'], resId16_2);

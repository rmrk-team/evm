...
contract RMRKCore is Context, IRMRKCore, AccessControl {
  ...
  */
    ...
    function nftOwnerOf(uint256 tokenId)
        public
        view
        virtual
  ...
  */
    ...
    function addChildToPending(Child memory _child, uint256 _tokenId) internal {
        if (_pendingChildren[_tokenId].length < 128) {
            ...
            //wawawewa
            _pendingChildren[_tokenId][
                _emptyIndexes[_tokenId][(_emptyIndexes[_tokenId].length - 1)]
            ] = _child;
    ...
    function removeChild(
        ...
        while (i < children.length) {
            if (
                ...
                children[i].tokenId == childTokenId
            ) {
  ...
  */
    ...
    function removeParent(
        ...
        require(
            ...
            "Attempt to remove uninitialized parent"
        );
  ...
  */
    ...
    function isApprovedOwnerOrNftOwner(address spender, uint256 tokenId)
        public
        view
        virtual
        returns (bool)
    ...
    function getTokenResource(uint256 tokenId, bytes8 resourceId)
        public
        view
        virtual
        returns (Resource memory)
    ...
    function getPriorities(uint256 tokenId)
        public
        view
        virtual

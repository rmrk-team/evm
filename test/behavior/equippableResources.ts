...
async function shouldBehaveLikeEquippableResources(
  ...
  describe('Resource storage', async function () {
    ...
    it('cannot get extended resources for non existing resource or non existing token', async function () {
      ...
      await expect(
        chunkyEquip.getExtendedResource(tokenId, resId + 1),
      ).to.be.revertedWithCustomError(chunkyEquip, 'RMRKTokenDoesNotHaveResource');
      await expect(
        chunkyEquip.getExtendedResource(tokenId + 1, resId),
      ).to.be.revertedWithCustomError(chunkyEquip, 'RMRKTokenDoesNotHaveResource');
    });
  ...
  describe('Adding resources', async function () {
    it('can add resource to token', async function () {
      ...
      if (chunkyEquip.getFullPendingExtendedResources === undefined) {
        ...
        expect(
          await renderUtils.getResourcesById(chunkyEquip.address, tokenId, pendingResources),
  ...
  describe('Accepting resources', async function () {
    it('can accept resource', async function () {
      ...
      await addResources([resId]);
      await chunkyEquip.addResourceToToken(tokenId, resId, 0);
      await expect(chunkyEquip.acceptResource(tokenId, resId))
      ...
      if (chunkyEquip.getFullExtendedResources === undefined) {
        ...
        expect(
          await renderUtils.getResourcesById(chunkyEquip.address, tokenId, activeResources),
    ...
    it('can accept multiple resources', async function () {
      ...
      await expect(chunkyEquip.acceptResource(tokenId, resId2))
        .to.emit(chunkyEquip, 'ResourceAccepted')
    ...
    it('can accept resource if approved', async function () {
      ...
      await chunkyEquip.addResourceToToken(tokenId, resId, 0);
      await chunkyEquip.connect(approvedAddress).acceptResource(tokenId, resId);
    ...
    it('cannot accept resource twice', async function () {
      ...
      await addResources([resId]);
      await chunkyEquip.addResourceToToken(tokenId, resId, 0);
      await chunkyEquip.acceptResource(tokenId, resId);

      await expect(chunkyEquip.acceptResource(tokenId, resId)).to.be.revertedWithCustomError(
        chunkyEquip,
    ...
    it('cannot accept resource if not owner', async function () {
      ...
      await chunkyEquip.addResourceToToken(tokenId, resId, 0);
      await expect(
        chunkyEquip.connect(addrs[1]).acceptResource(tokenId, resId),
    ...
    it('cannot accept non existing resource', async function () {
      const tokenId = await mint(chunky, owner.address);
      await expect(chunkyEquip.acceptResource(tokenId, 10)).to.be.revertedWithCustomError(
        chunkyEquip,
        'RMRKResourceNotFoundOnTokenPending',
      );
    });
  ...
  describe('Overwriting resources', async function () {
    ...
    it('can overwrite non existing resource to token, it could have been deleted', async function () {
      ...
      const tokenId = await mint(chunky, owner.address);
      await addResources([resId]);
      await chunkyEquip.addResourceToToken(tokenId, resId, 1);
      await chunkyEquip.acceptResource(tokenId, resId);
  ...
  describe('Rejecting resources', async function () {
    it('can reject resource', async function () {
      ...
      await expect(chunkyEquip.rejectResource(tokenId, resId)).to.emit(
        chunkyEquip,
        'ResourceRejected',
      );
    ...
    it('can reject resource and overwrites are cleared', async function () {
      ...
      await chunkyEquip.rejectResource(tokenId, resId2);
    ...
    it('cannot reject resource twice', async function () {
      ...
      await addResources([resId]);
      await chunkyEquip.addResourceToToken(tokenId, resId, 0);
      await chunkyEquip.rejectResource(tokenId, resId);

      await expect(chunkyEquip.rejectResource(tokenId, resId)).to.be.revertedWithCustomError(
        chunkyEquip,
        'RMRKResourceNotFoundOnTokenPending',
      );
    });
    ...
    it('cannot reject resource nor reject all if not owner', async function () {
      ...
      await expect(
        chunkyEquip.connect(addrs[1]).rejectResource(tokenId, resId),
      ).to.be.revertedWithCustomError(chunkyEquip, 'RMRKNotApprovedForResourcesOrOwner');
      await expect(
    ...
    it('cannot reject non existing resource', async function () {
      const tokenId = await mint(chunky, owner.address);
      await expect(chunkyEquip.rejectResource(tokenId, 10)).to.be.revertedWithCustomError(
        chunkyEquip,
        'RMRKResourceNotFoundOnTokenPending',
      );
    });
  ...
  describe('Approval Cleaning', async function () {
    it('cleans token and resources approvals on transfer', async function () {
      ...
      expect(await chunkyEquip.getApprovedForResources(tokenId)).to.eql(ADDRESS_ZERO);
    });
  ...
  async function addResourcesToToken(): Promise<number> {
    ...
    await chunkyEquip.addResourceToToken(tokenId, resId2, 0);
    await chunkyEquip.acceptResource(tokenId, resId);
    await chunkyEquip.acceptResource(tokenId, resId2);

...
async function shouldBehaveLikeERC721(name: string, symbol: string) {
  ...
  context('with minted tokens', function () {
    ...
    describe('transfers', function () {
      ...
      // only be useful when happened in nesting test code
      if (this.ctx.token['safeTransferNestingFrom(address,uint256,bytes)']) {
        describe('via safeTransferNestingFrom', function () {
          const safeTransferNestingFromWithData = function (
            token: Contract,
            from: string,
            to: string,
            tokenId: BigNumber,
            ...
            return token
              .connect(user)
              ['safeTransferNestingFrom(address,address,uint256,bytes)'](from, to, tokenId, data);
          };

          const safeTransferNestingFromWithoutData = function (
            token: Contract,
            from: string,
            to: string,
            tokenId: BigNumber,
            ...
            return token
              .connect(user)
              ['safeTransferNestingFrom(address,address,uint256)'](from, to, tokenId);
          };
          ...
          const shouldTransferSafely = function (transferFun: any, data: any) {
            ...
            describe('to a valid receiver contract', function () {
              ...
              it('calls onRMRKNestingReceived', async function () {
                receipt = await transferFun(
                  this.token,
                  owner.address,
                  receiver.address,
              ...
              it('calls onRMRKNestingReceived from approved', async function () {
                receipt = await transferFun(
                  this.token,
                  owner.address,
                  receiver.address,
          ...
          describe('with data', function () {
            shouldTransferSafely(safeTransferNestingFromWithData, data);
          });

          describe('without data', function () {
            shouldTransferSafely(safeTransferNestingFromWithoutData, '0x');
          });
        });
      }
    });
    ...
    describe('safe mint', function () {
      ...
      describe('approve', function () {
        ...
        context('when the address that receives the approval is the owner', function () {
          it('reverts', async function () {
            await expect(this.token.approve(owner.address, tokenId)).to.be.revertedWithCustomError(
              this.token,
    ...
    // only be useful when happened in nesting test code
    if (this.ctx.token['safeMintNesting(address,uint256,bytes)']) {
      describe('safe mint nesting', function () {
        const tokenId = fourthTokenId;
        const data = '0x42';
        ...
        describe('via safeMintNesting', function () {
          ...
          it('calls onERC721Received — with data', async function () {
            receiver = await this.RMRKNestingReceiver.deploy(
              RECEIVER_MAGIC_VALUE_RMRK_NESTING,
              Error.None,
            );
            await receiver.deployed();
            const receipt = await this.token['safeMintNesting(address,uint256,bytes)'](
              receiver.address,
              tokenId,
              data,
            );
          ...
          it('calls onERC721Received — without data', async function () {
            ...
            await receiver.deployed();
            const receipt = await this.token['safeMintNesting(address,uint256)'](
              receiver.address,
              tokenId,
            );
        ...
        context('to a receiver contract that reverts with message', function () {
          it('reverts', async function () {
            const revertingReceiver = await this.RMRKNestingReceiver.deploy(
              ethers.utils.hexZeroPad('0x42', 4),
              Error.RevertWithMessage,
            );
            await revertingReceiver.deployed();
        ...
        context('to a contract that does not implement the required function', function () {
          it('reverts', async function () {
            const nonReceiver = await this.commonERC721.deploy(
              RECEIVER_MAGIC_VALUE_RMRK_NESTING,
              Error.None,
            );
            await expect(

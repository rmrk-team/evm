...
describe('RMRKSoulbound exempt', async function () {
  ...
  it('can unnest child if soulbound exempt', async function () {
    ...
    await token.connect(owner).acceptChild(tokenId, token.address, otherTokenId);
    await token.setSoulboundExempt(otherTokenId);

    await token
      .connect(owner)
      .unnestChild(tokenId, owner.address, token.address, otherTokenId, false);
...
async function shouldBehaveLikeSoulboundNesting() {
  ...
  it('cannot unnest', async function () {
    ...
    await soulbound.connect(owner).acceptChild(tokenId, soulbound.address, childId);
    expect(
      soulbound
        .connect(owner)
        .unnestChild(tokenId, owner.address, soulbound.address, childId, false),
    ).to.be.revertedWithCustomError(soulbound, 'RMRKCannotTransferSoulbound');
  });
}

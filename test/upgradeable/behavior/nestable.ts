...
import { upgradeable } from '../../../typechain-types/contracts';
...
async function shouldBehaveLikeNestable(
  ...
  describe('Minting', async function () {
    ...
    it('cannot nest mint to non rmrk nestable receiver', async function () {
      const ERC721 = await ethers.getContractFactory('ERC721MockUpgradeable');
      const nonReceiver = await upgrades.deployProxy(ERC721, ['Non receiver', 'NR']);
  ...
  describe('Nest Transfer', async function () {
    ...
    it('cannot nest tranfer to contract if it does implement IERC6059', async function () {
      const ERC721 = await ethers.getContractFactory('ERC721MockUpgradeable');
      const nonNestable = await upgrades.deployProxy(ERC721, ['Non receiver', 'NR']);

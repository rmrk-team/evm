...
import { RMRKMultiAssetMockUpgradeable, RMRKMultiAssetRenderUtils } from '../../typechain-types';
...
async function singleFixture(): Promise<{
  token: RMRKMultiAssetMockUpgradeable;
  ...
  const token = <RMRKMultiAssetMockUpgradeable>(
    await singleUpgradeableFixtureWithArgs('RMRKMultiAssetMockUpgradeable', [name, symbol])
  );
  return { token, renderUtils };
}

describe('MultiAssetMock Other Behavior', async function () {
  let token: RMRKMultiAssetMockUpgradeable;

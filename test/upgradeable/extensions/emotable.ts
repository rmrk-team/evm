...
async function multiAssetEmotableFixture() {
  const factory = await ethers.getContractFactory('RMRKMultiAssetEmotableMockUpgradeable');
...
async function emoteTrackerFixture() {
  const factory = await ethers.getContractFactory('RMRKEmoteTrackerMockUpgradeable');
  const erc721Factory = await ethers.getContractFactory('ERC721MockUpgradeable');
  const emoteTracker = await upgrades.deployProxy(factory, []);
  const tokenA = await upgrades.deployProxy(erc721Factory, ['Token A', 'TKA']);
  const tokenB = await upgrades.deployProxy(erc721Factory, ['Token B', 'TKB']);

...
describe('ERC721', async () => {
  ...
  describe('with minted tokens', function () {
    beforeEach(async function () {
      ...
      token = await Erc721.deploy(name, symbol);
    ...
    describe('metadata', function () {
      ...
      describe('token URI', function () {
        it('return empty SignerWithAddress by default', async function () {
          expect(await token.tokenURI(firstTokenId)).to.be.equal('');
        });

        it('reverts when queried for non existent token id', async function () {
          await expect(token.tokenURI(nonExistentTokenId)).to.be.revertedWithCustomError(
            token,
    ...
    describe('balanceOf', function () {
      context('when the given address owns some tokens', function () {
        it('returns the amount of tokens owned by the given address', async function () {
          expect(await token.balanceOf(owner.address)).to.eql(BigNumber.from(2));
        });
      });

      context('when the given address does not own any tokens', function () {
        it('returns 0', async function () {
          expect(await token.balanceOf(others[0].address)).to.eql(BigNumber.from(0));
        });
      });

      context('when querying the zero address', function () {
        it('throws', async function () {
          await expect(token.balanceOf(ethers.constants.AddressZero)).to.be.revertedWithCustomError(
            token,
    ...
    describe('ownerOf', function () {
      ...
      context('when the given token ID was not tracked by this token', function () {
        ...
        it('reverts', async function () {
          await expect(token.ownerOf(tokenId)).to.be.revertedWithCustomError(
            token,
            'ERC721InvalidTokenId',
          );
        });
    ...
    describe('transfers', function () {
      ...
      const transferWasSuccessful = function (owner: SignerWithAddress, tokenId: BigNumber) {
        ...
        it('clears the approval for the token ID', async function () {
          expect(await token.getApproved(tokenId)).to.be.equal(ethers.constants.AddressZero);
        });

        it('adjusts owners balances', async function () {
          expect(await token.balanceOf(owner.address)).to.eql(BigNumber.from(1));
        });
      };
      ...
      const shouldTransferTokensByUsers = function (transferFunction: any) {
        context('when called by the owner', function () {
          beforeEach(async function () {
            receipt = await transferFunction(owner.address, toWhom.address, tokenId, owner);
          });
        ...
        context('when called by the approved individual', function () {
          beforeEach(async function () {
            receipt = await transferFunction(owner.address, toWhom.address, tokenId, approved);
          });
          afterEach(async function () {
            transferWasSuccessful(owner, tokenId);
          });
        ...
        context('when called by the operator', function () {
          beforeEach(async function () {
            receipt = await transferFunction(owner.address, toWhom.address, tokenId, operator);
          });
          afterEach(async function () {
            transferWasSuccessful(owner, tokenId);
          });
        ...
        context('when called by the owner without an approved user', function () {
          beforeEach(async function () {
            await token.approve(ethers.constants.AddressZero, tokenId);
            receipt = await transferFunction(owner.address, toWhom.address, tokenId, operator);
          });
          afterEach(async function () {
            transferWasSuccessful(owner, tokenId);
        ...
        context('when sent to the owner', function () {
          beforeEach(async function () {
            receipt = await transferFunction(owner.address, owner.address, tokenId, owner);
          });
        ...
        context('when the address of the previous owner is incorrect', function () {
          it('reverts', async function () {
            await expect(
              transferFunction(others[0].address, others[0].address, tokenId, owner),
            ).to.be.revertedWithCustomError(token, 'ERC721TransferFromIncorrectOwner');
          });
        });
        ...
        context('when the sender is not authorized for the token id', function () {
          it('reverts', async function () {
            await expect(
              transferFunction(owner.address, others[0].address, tokenId, others[0]),
        ...
        context('when the given token ID does not exist', function () {
          it('reverts', async function () {
            await expect(
              transferFunction(owner.address, others[0].address, nonExistentTokenId, owner),
        ...
        context('when the address to transfer the token to is the zero address', function () {
          it('reverts', async function () {
            await expect(
              transferFunction(owner.address, ethers.constants.AddressZero, tokenId, owner),
            ).to.be.revertedWithCustomError(token, 'ERC721TransferToTheZeroAddress');
          });
        });
      };
        ...
        const shouldTransferSafely = function (transferFun: any, data: any) {
          ...
          describe('to a valid receiver contract', function () {
            ...
            it('calls onERC721Received', async function () {
              receipt = await transferFun(owner.address, receiver.address, tokenId, owner);
            ...
            it('calls onERC721Received from approved', async function () {
              receipt = await transferFun(owner.address, receiver.address, tokenId, approved);
              expect(receipt)
                .to.emit(receiver, 'Received')
            ...
            describe('with an invalid token id', function () {
              it('reverts', async function () {
                await expect(
                  transferFun(owner.address, receiver.address, nonExistentTokenId, owner),
                ).to.be.revertedWithCustomError(token, 'ERC721InvalidTokenId');
              });
            });
          });
    ...
    describe('safe mint', function () {
      ...
      describe('via safeMint', function () {
        ...
        it('calls onERC721Received — with data', async function () {
          ...
          receiver = await ERC721Receiver.deploy(RECEIVER_MAGIC_VALUE, Error.None);
          await receiver.deployed();
          const receipt = await token['safeMint(address,uint256,bytes)'](
            receiver.address,
            tokenId,
            data,
          );
      ...
      it('calls onERC721Received — without data', async function () {
        ...
        receiver = await ERC721Receiver.deploy(RECEIVER_MAGIC_VALUE, Error.None);
        await receiver.deployed();
        const receipt = await token['safeMint(address,uint256)'](receiver.address, tokenId);
      ...
      context('to a receiver contract returning unexpected value', function () {
        it('reverts', async function () {
          ...
          await expect(
            token['safeMint(address,uint256)'](invalidReceiver.address, tokenId),
          ).to.be.revertedWithCustomError(token, 'ERC721TransferToNonReceiverImplementer');
        });
      });
      ...
      context('to a receiver contract that reverts with message', function () {
        it('reverts', async function () {
          ...
          await revertingReceiver.deployed();
          await expect(
            token.connect(owner)['safeMint(address,uint256)'](revertingReceiver.address, tokenId),
          ).to.be.revertedWith('ERC721ReceiverMock: reverting');
        });
      });
      ...
      context('to a contract that does not implement the required function', function () {
        it('reverts', async function () {
          const nonReceiver = token;
          await expect(
            token['safeMint(address,uint256)'](nonReceiver.address, tokenId),
          ).to.be.revertedWith('ERC721: transfer to non ERC721Receiver implementer');
        });
      });
      ...
      describe('approve', function () {
        ...
        const itApproves = function (address: string) {
          it('sets the approval for the target address', async function () {
            expect(await token.getApproved(tokenId)).to.be.equal(address);
          });
        };

        const itEmitsApprovalEvent = function (address: string) {
          it('emits an approval event', async function () {
            expect(receipt).to.emit(token, 'Approval').withArgs(owner.address, address, tokenId);
          });
        };
        ...
        context('when clearing approval', function () {
          context('when there was no prior approval', function () {
            beforeEach(async function () {
              receipt = await token.approve(ethers.constants.AddressZero, tokenId);
            });
        ...
        context('when approving a non-zero address', function () {
          context('when there was no prior approval', function () {
            beforeEach(async function () {
              receipt = await token.approve(approved.address, tokenId);
            });
          ...
          context('when there was a prior approval to a different address', function () {
            beforeEach(async function () {
              await token.approve(anotherApproved.address, tokenId);
              receipt = await token.approve(anotherApproved.address, tokenId);
            });
        ...
        context('when the address that receives the approval is the owner', function () {
          it('reverts', async function () {
            await expect(token.approve(owner.address, tokenId)).to.be.revertedWithCustomError(
              token,
        ...
        context('when the sender does not own the given token ID', function () {
          it('reverts', async function () {
            await expect(
              token.connect(others[0]).approve(approved.address, tokenId),
        ...
        context('when the sender is approved for the given token ID', function () {
          it('reverts', async function () {
            await token.approve(approved.address, tokenId);
            await expect(
              token.connect(approved).approve(anotherApproved.address, tokenId),
            ).to.be.revertedWithCustomError(
              token,
              'ERC721ApproveCallerIsNotOwnerNorApprovedForAll',
            );
        ...
        context('when the sender is an operator', function () {
          beforeEach(async function () {
            await token.setApprovalForAll(operator.address, true);
            receipt = await token.connect(operator).approve(approved.address, tokenId);
          });
        ...
        context('when the given token ID does not exist', function () {
          it('reverts', async function () {
            await expect(
              token.connect(operator).approve(approved.address, nonExistentTokenId),
            ).to.be.revertedWithCustomError(token, 'ERC721InvalidTokenId');
          });
        });
      });
      ...
      describe('setApprovalForAll', function () {
        context('when the operator willing to approve is not the owner', function () {
          context('when there is no operator approval set by the sender', function () {
            it('approves the operator', async function () {
              ...
              expect(await token.isApprovedForAll(owner.address, operator.address)).to.equal(true);
            });

            it('emits an approval event', async function () {
              const receipt = await token.setApprovalForAll(operator.address, true);
          ...
          context('when the operator was set as not approved', function () {
            beforeEach(async function () {
              await token.setApprovalForAll(operator.address, false);
            });
            ...
            it('can unset the operator approval', async function () {
              ...
              expect(await token.isApprovedForAll(owner.address, operator.address)).to.equal(false);
            });
          });
        ...
        context('when the operator is the owner', function () {
          it('reverts', async function () {
            ...
            ).to.be.revertedWithCustomError(token, 'ERC721ApproveToCaller');
          });
        });
      });
      ...
      describe('getApproved', async function () {
        context('when token is not minted', async function () {
          it('reverts', async function () {
            await expect(token.getApproved(nonExistentTokenId)).to.be.revertedWithCustomError(
              token,
              'ERC721InvalidTokenId',
            );
          });
        ...
        context('when token has been minted ', async function () {
          it('should return the zero address', async function () {
            expect(await token.getApproved(firstTokenId)).to.be.equal(ethers.constants.AddressZero);
          });

          context('when account has been approved', async function () {
            beforeEach(async function () {
              await token.approve(approved.address, firstTokenId);
            });

            it('returns approved account', async function () {
              expect(await token.getApproved(firstTokenId)).to.be.equal(approved.address);
            });
          });
        });
      });
  ...
  describe('_mint(address, uint256)', function () {
    ...
    it('reverts with a null destination address', async function () {
      await expect(
        token.mint(ethers.constants.AddressZero, firstTokenId),
      ).to.be.revertedWithCustomError(token, 'ERC721MintToTheZeroAddress');
    });

    context('with minted token', async function () {
      beforeEach(async function () {
        receipt = await token.mint(owner.address, firstTokenId);
      });
      ...
      it('reverts when adding a token id that already exists', async function () {
        await expect(token.mint(owner.address, firstTokenId)).to.be.revertedWithCustomError(
          token,
  ...
  describe('_burn', function () {
    ...
    it('reverts when burning a non-existent token id', async function () {
      await expect(token.burn(nonExistentTokenId)).to.be.revertedWithCustomError(
        token,
        'ERC721InvalidTokenId',
      );
    });
    ...
    context('with minted tokens', function () {
      ...
      context('with burnt token', function () {
        beforeEach(async function () {
          receipt = await token.burn(firstTokenId);
        });
        ...
        it('deletes the token', async function () {
          expect(await token.balanceOf(owner.address)).to.eql(BigNumber.from(1));
          await expect(token.ownerOf(firstTokenId)).to.be.revertedWithCustomError(
            token,
            'ERC721InvalidTokenId',
          );
        });
        ...
        it('reverts when burning a token id that has been deleted', async function () {
          await expect(token.burn(firstTokenId)).to.be.revertedWithCustomError(
            token,
            'ERC721InvalidTokenId',
          );
        });

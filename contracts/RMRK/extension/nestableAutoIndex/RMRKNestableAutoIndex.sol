...
contract RMRKNestableAutoIndex is IRMRKNestableAutoIndex, RMRKNestable {
    ...
    function _beforeAddChild(
        ...
        _pendingChildrenIndex[tokenId][childAddress][childId] = _pendingChildren[tokenId].length;
    }
    ...
    function _beforeTransferChild(
        ...
        if (isPending)
            _removePendingChildIndex(tokenId, childIndex);
        else
            _removeActiveChildIndex(tokenId, childIndex);
    }

    function _removePendingChildIndex(uint256 parentId, uint256 childIndex) private {
    ...
    function _removeActiveChildIndex(uint256 parentId, uint256 childIndex) private {
        // We need to update the childIndex for the last child since it will be swapped when removing this
